...
[![npm version](https://badge.fury.io/js/@swenkerorg/earum-nostrum.svg)](https://badge.fury.io/js/@swenkerorg/earum-nostrum) [![downloads](https://badgen.net/npm/dy/@swenkerorg/earum-nostrum)](https://npm-stat.com/charts.html?package=@swenkerorg/earum-nostrum)
...
`@swenkerorg/earum-nostrum` uses a heavily-modified fork of [simple-markdown](https://github.com/Khan/simple-markdown) as its parsing engine and extends it in a number of ways to make your life easier. Notably, this package offers the following additional benefits:
...
Install `@swenkerorg/earum-nostrum` with your favorite package manager.

```shell
npm i @swenkerorg/earum-nostrum
```

## Usage

`@swenkerorg/earum-nostrum` exports a React component by default for easy JSX composition:

ES6-style usage\*:

```jsx
import Markdown from '@swenkerorg/earum-nostrum'
...
Any conflicts between passed `props` and the specific properties above will be resolved in favor of `@swenkerorg/earum-nostrum`'s code.
...
One of the most interesting use cases enabled by the HTML syntax processing in `@swenkerorg/earum-nostrum` is the ability to use any kind of element, even ones that aren't real HTML tags like React component classes.
...
`@swenkerorg/earum-nostrum` also handles JSX interpolation syntax, but in a minimal way to not introduce a potential attack vector. Interpolations are sent to the component as their raw string, which the consumer can then `eval()` or process as desired to their security needs.
...
````tsx
import Markdown, { RuleType } from '@swenkerorg/earum-nostrum'
...
To get around this, `@swenkerorg/earum-nostrum` left-trims approximately as much whitespace as the first line inside the HTML block. So for example:
...
If desired, the compiler function is a "named" export on the `@swenkerorg/earum-nostrum` module:

```jsx
import { compiler } from '@swenkerorg/earum-nostrum'
import React from 'react'
import { render } from 'react-dom'
...
See [Github Releases](https://github.com/probablyup/@swenkerorg/earum-nostrum/releases).
